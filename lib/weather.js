    ...
    update: function(weatherData, cb) {
      var conditionsCall = function(weatherData) {
        ...
        http.get(url, function(res){
          ...
          res.on('end', function(){
            ...
            weatherData.data.weather = data.current_observation.weather;
            weatherData.data.icon = getWeatherIcon(data.current_observation.icon);
            weatherData.data.temp = Math.round(data.current_observation.temp_c);
      ...
      var forecastCall = function(weatherData) {
        ...
        http.get(url, function(res){
          ...
          res.on('end', function(){
            data = JSON.parse(data);
            weatherData.data.forecastToday = data.forecast.txt_forecast.forecastday[0].fcttext_metric;
            weatherData.data.forecastTonight = data.forecast.txt_forecast.forecastday[1].fcttext_metric;
            deferred.resolve(weatherData);
          });
        });
        return deferred.promise;
